...
int Settings::getRand(int min, int max) {
...
// For simulating power consumption.
void Settings::on_battery_lvl_valueChanged(int value) {
    ui->battery_lvl->setValue(value);
    ui->battery_label->setText(QString::number(value) + "%");
}

void Settings::on_off_button_clicked() { close(); }

// Updating battery charge.
void Settings::setBattery(int b, int cntr) {
    battery = b;
    battery_cntr = cntr;
    on_battery_lvl_valueChanged(b);
}

void Settings::setPwr(int p) { pwr_usage = p; }
